...
public class AppPaySubmit {
    ...
    private static String buildRequestMysign(Map<String, String> sPara) {
        String prestr = AppPayCore.createLinkString(sPara); //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
        String mysign = "";
        if(AppPayConfig.WECHAT_SIGN_TYPE.equals("MD5") ) {
            mysign = AppPayCore.sign(prestr, AppPayConfig.WECHAT_KEY, AppPayConfig.WECHAT_INPUT_CHARSET);
        }
        return mysign;
    }
    ...
    private static Map<String, String> buildRequestPara(Map<String, String> sParaTemp) {
        //除去数组中的空值和签名参数
        Map<String, String> sPara = AppPayCore.paraFilter(sParaTemp);
        //生成签名结果
