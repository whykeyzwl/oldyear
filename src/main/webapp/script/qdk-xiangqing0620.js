...
window.onload = function() {
	...
	function is_wb(){
		...
		if (ua.match(/WeiBo/i) == "weibo") {
			window.open("http://m.qingdouke.com/wap/wbtologin.html?reurl=" + window.location.href);
		} else {
	...
	function weixinLogin() {
		if (_user == "") {
			location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxcb0f747f6e77fe7f&redirect_uri=http%3A%2F%2Fm.qingdouke.com%2Fwap%2Fwxlogin.html%3Freurl%3Dhttp%3A%2F%2Fm.qingdouke.com%2Fview%2Fqdk-xiangqing.html?picId=" + _albumId + "&time=" + nowTime + "&response_type=code&scope=snsapi_userinfo&state=" + (Math.random() * 9 + 1) * 100000 + "#wechat_redirect";
			getwechatpassportId();
		} else {
			// 调用微信支付接口
			wechatPay()
	...
	function bindChange() {
		...
		$(".goPic").live("click", function() {
			location.href = "./qdk-pic.html?albumId=" + _albumId+"&time="+new Date().getTime();
		})
		// 点击微信支付
